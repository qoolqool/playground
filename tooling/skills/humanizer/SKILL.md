...
name: humanizer
description: |
  Remove signs of AI-generated writing from text to make it sound more natural
  and human-written. Based on Wikipedia's comprehensive "Signs of AI writing"
...
# ASCII Diagram Repair

Use this skill whenever an existing ASCII diagram has broken arrows, misalignment, or unequal box margins.

## Instructions for ASCII Alignment

1. Locate the misaligned vertical (`|`) or horizontal (`─`) lines.
2. Ensure every line within the ` ```text ` or ` ```ascii ` block has the exact same character width.
3. Use uniform padding spaces instead of tabs.
4. If a block is altered, recalculate the length of connecting arrows to prevent broken paths.
5. Use standardized Unicode box-drawing characters (┌, ┐, └, ┘, ├, ┤, ┼) instead of hyphens and slashes for maximum stability across terminal widths.
