...
function collectSummary(messages: Message[]): { filesChanged: string[]; testsRan: boolean } {
 ...
 if ((part.name === "write" || part.name === "edit") && typeof (part.arguments as Record<string, unknown>)?.path === "string") {
 files.add((part.arguments as Record<string, unknown>).path as string);
 }
 if (part.name === "bash") {
 const cmd = (part.arguments as Record<string, unknown>)?.command;
...
async function runSingleAgent(
 ...
 currentResult.usage.cost += usage.cost || 0;
 currentResult.usage.contextTokens = usage.totalTokens || 0;
 }
 if (!currentResult.model && (msg as Record<string, unknown>).model) currentResult.model = (msg as Record<string, unknown>).model as string;
...
export default function (pi: ExtensionAPI) {
 ...
 if (currentResult) {
 const allResults = [...results, currentResult as unknown as SingleResult];
 onUpdate({
 ...
 allResults[index] = partial.details.results[0] as unknown as SingleResult;
 emitParallelUpdate();
 }
 },
